    ...
    class Program
    {
        ...
        static async Task TestCompressedCache()
        {
            ...
            try
            {
                ...
                // Test multiple entries for better statistics
                for (int i = 0; i < 5; i++)
                {
                    var data = new TestObject
                    {
                        Id = i,
                        Name = $"Test object {i} with repeated text to improve compression",
                        Items = Enumerable.Repeat($"Repeated item {i}", 10).ToList()
                    };
                    await cache.SetAsync($"test-key-{i}", data);
                }

                // Get updated statistics
                stats = await cache.GetStatsAsync();
                Console.WriteLine($"   Final cache entries: {stats.TotalEntries}");
                Console.WriteLine($"   Final compressed size: {stats.TotalCompressedSizeFormatted}");
                Console.WriteLine($"   Final compression ratio: {stats.AverageCompressionRatio}%");
        ...
        static async Task TestProjectCacheManager()
        {
            ...
            try
            {
                ...
                // Test project retrieval (cache hit)
                ...
                {
                    Console.WriteLine("   ✅ Project entry retrieved from cache (cache hit)");
                }
                else
                {
                ...
                var missResult = await projectCache.GetProjectAsync("non-existent.csproj", "net8.0");
                if (missResult == null)
                {
                    Console.WriteLine("   ✅ Cache miss handled correctly");
                }
